...
const PaymentMenu = () => {
  ...
  useEffect(() => {
    ...
    const handleContaFechada = (data: { contaId: number }) => {
      if (data.contaId == contaId) {
            localStorage.removeItem("conta")
      }
    };

    socket.on("contaFechada", handleContaFechada);
    return () => {
      socket.off("contaFechada", handleContaFechada);
    };
  }, [socket]);
  ...
  const requestPayment = async () => {
    ...
    try {
      ...
      const donoConta = conta.donoConta
      const numeroMesa = mesa.numero

      if (socket) {
        // Agora estamos passando mesa e conta como objetos completos
        socket.emit("solicitandoConta",{ numeroMesa, idConta, donoConta});
